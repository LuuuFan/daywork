...
def connecting_routes
  ...
  execute(<<-SQL)
    ...
    FROM routes
    WHERE stop_id IN ( SELECT id FROM stops WHERE name = 'London Road' OR name = 'Craiglockhart' )
...
def start_at_craiglockhart
  ...
  execute(<<-SQL)
    SELECT stops.name, routes.company, routes.num
    FROM stops
    JOIN routes ON stops.id = routes.stop_id
    JOIN (SELECT DISTINCT a.num, a.company
          FROM routes a
          JOIN routes b ON (a.company = b.company AND a.num = b.num)
          JOIN stops ON stops.id = a.stop_id
          WHERE stops.name = 'Craiglockhart') AS company_stop
      ON (company_stop.company = routes.company AND company_stop.num = routes.num)
  SQL
end
...
def craiglockhart_to_sighthill
  ...
  execute(<<-SQL)
    ...
    -- FROM (SELECT stops.name, routes.company, routes.num
    --       FROM stops
    --       JOIN routes ON stops.id = routes.stop_id
    --       JOIN (SELECT DISTINCT a.num, a.company
    --             FROM routes a
    ...
    --         ON (company_stop.company = routes.company AND company_stop.num = routes.num)
    --       ) AS start
    -- JOIN (SELECT stops.name, routes.company, routes.num
    --       FROM stops
    --       JOIN routes ON stops.id = routes.stop_id
    --       JOIN (SELECT DISTINCT a.num, a.company
    --             FROM routes a
    --             JOIN routes b ON (a.company = b.company AND a.num = b.num)
    --             JOIN stops ON stops.id = a.stop_id
    --             WHERE stops.name = 'Sighthill') AS company_stop
    --         ON (company_stop.company = routes.company AND company_stop.num = routes.num)
    --
    -- ) AS finish
    ...
    SELECT start.num, start.company, start.name, finish.num, finish.company
    FROM start JOIN finish ON start.name = finish.name
  SQL
end
